...
The results of the models tested are not particularly ideal for use however some are stronger for use than others. All the models had issues with uneven precision scores between high and low risk loans with high accuracy for only low risk loans. The recall scores between the oversampling models, undersampling model and combination sampling model were are all fairly low showing poor accuracy in predicting positive loans. There was improvement in the balanced random forest classifier model in accurately predicting credit risk and positive loans but the biggest improvement was with the easy ensemble AdaBoost classifier model. The easy ensemble AdaBoost classifier model had the best balanced accuracy score, precision scores and recall scores. Ultimately based on these results, the best recommendation for a model to use would be the easy ensemble AdaBoost classifier model.
